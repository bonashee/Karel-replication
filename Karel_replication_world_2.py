...
def main():
    ...
    paint_one_rect(karel)
    while not front_is_clear(karel):
        turn_right(karel)
        paint_one_rect(karel)
    # for i in range(2):
    #     put_beeper(karel)
...
def paint_one_rect(karel):
    """
    Makes Karel go along three wall of a rectangle and put beepers every corner bordered with a wall

    pre-condition: Karel stands at the beginning of a wall(first wall out of four). The wall is on the Karel left.

    Post-condition: Karel stands at the end the third wall(out of four). All corners bordered with rectangle include
    beepers.
    """
    for i in range(2):
        paint_one_side(karel)
        go_around_the_corner(karel)
    paint_one_side(karel)



def paint_one_side(karel):
    """
    Makes Karel do along the rectangle wall, dropping a beeper before each step it takes.

    Pre-condition: the rectangle side is on the Karel's left. Karel stand at the beginning of the rectangle side.

    Post-condition: Karel is facing the same direction as before, each corner along the rectangle side has a beeper
    added to it, Karel is standing a step away the end of the rectangle side
    """
    while not left_is_clear(karel):
        put_beeper(karel)
        move(karel)

def go_around_the_corner(karel):
    """
    Makes Karel turn around rectangle  corner to taught the next side.

    Pre-condition: Karel is standing a step away the end of the rectangle side. The next side if on Karel's left

    Post-condition: the rectangle side is on the Karel's left. Karel stand at the beginning of the rectangle side.
    """
    turn_left(karel)
    move(karel)
...
def make_walls(canvas):
    ...
    """
    walls = []
    walls.append(canvas.create_line(1 * SQUARE_SIZE, 3 * SQUARE_SIZE, 1 * SQUARE_SIZE, 6 * SQUARE_SIZE))
    walls.append(canvas.create_line(1 * SQUARE_SIZE, 3 * SQUARE_SIZE, 5 * SQUARE_SIZE, 3 * SQUARE_SIZE))
    walls.append(canvas.create_line(1 * SQUARE_SIZE, 6 * SQUARE_SIZE, 5 * SQUARE_SIZE, 6 * SQUARE_SIZE))
    walls.append(canvas.create_line(5 * SQUARE_SIZE, 3 * SQUARE_SIZE, 5 * SQUARE_SIZE, 6 * SQUARE_SIZE))

    walls.append(canvas.create_line(6 * SQUARE_SIZE, 6 * SQUARE_SIZE, 6 * SQUARE_SIZE, 8 * SQUARE_SIZE))
    walls.append(canvas.create_line(9 * SQUARE_SIZE, 6 * SQUARE_SIZE, 9 * SQUARE_SIZE, 8 * SQUARE_SIZE))
    walls.append(canvas.create_line(6 * SQUARE_SIZE, 6 * SQUARE_SIZE, 9 * SQUARE_SIZE, 6 * SQUARE_SIZE))
    walls.append(canvas.create_line(6 * SQUARE_SIZE, 8 * SQUARE_SIZE, 9 * SQUARE_SIZE, 8 * SQUARE_SIZE))

    walls.append(canvas.create_line(9 * SQUARE_SIZE, 2 * SQUARE_SIZE, 9 * SQUARE_SIZE, 5 * SQUARE_SIZE))
    walls.append(canvas.create_line(11 * SQUARE_SIZE, 2 * SQUARE_SIZE, 11 * SQUARE_SIZE, 5 * SQUARE_SIZE))
    walls.append(canvas.create_line(9 * SQUARE_SIZE, 2 * SQUARE_SIZE, 11 * SQUARE_SIZE, 2 * SQUARE_SIZE))
    walls.append(canvas.create_line(9 * SQUARE_SIZE, 5 * SQUARE_SIZE, 11 * SQUARE_SIZE, 5 * SQUARE_SIZE))
...
def create_karel(canvas, walls, beepers):
    ...
    rect_start_x = 4 * SQUARE_SIZE
    rect_start_y = 2 * SQUARE_SIZE
